...
@Component({
    selector: 'apos-day-book-entry',
    templateUrl: './day-book-entry.component.html'
})
export class DayBookEntryComponent implements OnInit, OnDestroy {
...
currentAccount: any;
    ...
    loadAll() {
        this.dayBookEntryService.query({
            page: this.page - 1,
            size: this.itemsPerPage,
            sort: this.sort()}).subscribe(
                (res: HttpResponse<DayBookEntry[]>) => this.onSuccess(res.body, res.headers),
    ...
    transition() {
        this.router.navigate(['/day-book-entry'], {queryParams:
            {
                page: this.page,
                size: this.itemsPerPage,
    ...
    clear() {
        this.page = 0;
        this.router.navigate(['/day-book-entry', {
            page: this.page,
            sort: this.predicate + ',' + (this.reverse ? 'asc' : 'desc')
        }]);
        this.loadAll();
